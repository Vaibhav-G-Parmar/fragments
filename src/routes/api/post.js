...
module.exports = async (req, res) => {
  ...
  if (Fragment.isSupportedType(req.get('Content-Type'))) {
    try {
      ...
      const fragment = new Fragment({ownerId: req.user, type: req.get('Content-Type'), size: req.body.length})
      ...
      await fragment.setData(req.body)             //setting the fragment's data in the database
